...
const WeFourGroup = () => {
  ...
  const getMessage = async () => {
    const subscriber = firestore()
      .collection('weFourGroup')
      .doc('weFourGroupMessages')
      .collection('messages')
  ...
  const onSend = useCallback(
    async (messages = []) => {
      ...
      if (uri !== '') {
        ...
        const reference = storage().ref(`/weFourGroupImages/${photoName}`);
        const pathToFile = result;

        // uploads file
        await reference.putFile(pathToFile);
        url = await storage()
          .ref(`/weFourGroupImages/${photoName}`)
          .getDownloadURL();
        ...
        const reference = storage().ref(
          `/weFourGroupDocuments/${documentName}`,
        );
        ...
        url = await storage()
          .ref(`/weFourGroupDocuments/${documentName}`)
          .getDownloadURL();
        ...
        const reference = storage().ref(`/weFourGroupVideos/${videoName}`);
        ...
        await reference
          ...
          .catch(e => console.log(e));
        url = await storage()
          .ref(`/weFourGroupVideos/${videoName}`)
          .getDownloadURL();
              ...
              datas.map(
                async el =>
                  await sendNotifications(
                    el.token,
                    `New Message in 𝖂𝖊 կ𝖔𝖚𝖗 Group From ${user.tname}`,
                    msg.text,
                  ),
              );
              setDocumentName('');
            ...
            .then(async () => {
              if (documentSnapshot._data.photoName !== '') {
                try {
                  await storage()
                    .ref(
                      '/weFourGroupImages/' + documentSnapshot._data.photoName,
                    )
                    .delete()
                ...
                try {
                  await storage()
                    .ref(
                      '/weFourGroupVideos/' + documentSnapshot._data.videoName,
                    )
                    .delete()
