      ...
      if (photoName) {
        ...
        await firestore()
          ...
          .then(async () => {
            ...
            await notifyAll(noticeTitle, body).then(async () => {
              setNoticeText('');
              setShowLoader(false);
              setshowAddNotice(true);
              showToast('success', 'Notice Added Successfully!');
  ...
  const deleteData = async el => {
    ...
    await firestore()
      ...
      .then(async () => {
        ...
        let noticeTitle = `One Notice Deleted By ${user.tname}`;
        let body = el.title;
        await notifyAll(noticeTitle, body)
          .then(() => {
            setShowLoader(false);
    ...
    if (editTitle !== '' && editNoticeText !== '') {
      ...
      await firestore()
        ...
        .then(async () => {
          ...
          setAllNotices(newData);
          setNoticeUpdateTime(Date.now());
          let noticeTitle = `New Notice Edited By ${user.tname}`;
          let body = `Title: ${editTitle}, Notice: ${editNoticeText}`;
