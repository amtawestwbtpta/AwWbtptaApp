...
const WbtptaWorkingChatRoom = () => {
  ...
  const getMessage = async () => {
    const subscriber = firestore()
      .collection('wbtptaWorkingGroupChat')
      .doc('wbtptaWorkingGroupChatMessages')
      .collection('messages')
    ...
    async (messages = []) => {
      ...
      if (uri !== '') {
        ...
        const reference = storage().ref(
          `/wbtptaWorkingGroupChatImages/${photoName}`,
        );
        ...
        url = await storage()
          .ref(`/wbtptaWorkingGroupChatImages/${photoName}`)
          .getDownloadURL();
        ...
        const reference = storage().ref(
          `/wbtptaWorkingGroupChatDocuments/${documentName}`,
        );
        ...
        url = await storage()
          .ref(`/wbtptaWorkingGroupChatDocuments/${documentName}`)
          .getDownloadURL();
        ...
        const reference = storage().ref(
          `/wbtptaWorkingGroupChatVideos/${videoName}`,
        );
        const pathToFile = videoUri;
        ...
        await reference
          ...
          .catch(e => console.log(e));
        url = await storage()
          .ref(`/wbtptaWorkingGroupChatVideos/${videoName}`)
          .getDownloadURL();
              ...
              datas.map(
                async el =>
                  await sendNotifications(
                    el.token,
                    `New Message in WBTPTA WORKING GROUP From ${user.tname}`,
                    msg.text,
                  ),
              );
              setDocumentName('');
            ...
            .then(async () => {
              if (documentSnapshot._data.photoName !== '') {
                try {
                  await storage()
                    .ref(
                      '/wbtptaWorkingGroupChatImages/' +
                        documentSnapshot._data.photoName,
                    )
                    .delete()
                ...
                try {
                  await storage()
                    .ref(
                      '/wbtptaWorkingGroupChatDocuments/' +
                        documentSnapshot._data.documentName,
                    )
                    .delete()
                ...
                try {
                  await storage()
                    .ref(
                      '/wbtptaWorkingGroupChatVideos/' +
                        documentSnapshot._data.videoName,
                    )
                    .delete()
  ...
  return (
    <ImageBackground
      ...
      <View
        ...
        />
        <Text style={styles.title}>WBTPTA WORKING GROUP</Text>
      </View>
...
export default WbtptaWorkingChatRoom;
