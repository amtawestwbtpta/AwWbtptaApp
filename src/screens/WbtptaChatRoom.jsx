  ...
  const onSend = useCallback(
    async (messages = []) => {
      ...
      if (uri !== '') {
        ...
        const reference = storage().ref(`/wbtptaGroupChatImages/${photoName}`);
        const pathToFile = result;

        // uploads file
        await reference.putFile(pathToFile);
        url = await storage()
          .ref(`/wbtptaGroupChatImages/${photoName}`)
          .getDownloadURL();
        ...
        const reference = storage().ref(
          `/wbtptaGroupChatDocuments/${documentName}`,
        );
        ...
        url = await storage()
          .ref(`/wbtptaGroupChatDocuments/${documentName}`)
          .getDownloadURL();
        ...
        const reference = storage().ref(`/wbtptaGroupChatVideos/${videoName}`);
        ...
        await reference
          ...
          .catch(e => console.log(e));
        url = await storage()
          .ref(`/wbtptaGroupChatVideos/${videoName}`)
          .getDownloadURL();
      ...
      await firestore()
        ...
        .add(myMsg)
        .then(async () => {
          let noticeTitle = `New Message in AMTA WEST GROUP From ${user.tname}`;
          let body = myMsg.text;
